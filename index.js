...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6285939888897:+62 859-3988-8897\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '#'
blocked = []
limitawal = '30'
...
async function starts() {
	...
	fs.existsSync('./Devils.json') && client.loadAuthInfo('./Devils.json')
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6285939888897@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '6285939888897@s.whatsapp.net'
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
				...
				case 'ytmp3':
					...
					anu = await fetchJson(`https://mhankbarbar.herokuapp.com/api/yta?url=${args[0]}`, {method: 'get'})
				...
				case 'ytmp4':
					if (args.length < 1) return reply('Urlnya mana um?')
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://mhankbarbar.herokuapp.com/api/ytv?url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				case 'tulis':
					...
					teks = body.slice(7)
					reply(mess.wait)
					anu = await fetchJson(`https://api.zeks.xyz/api/nulis?text=${teks}&apikey=${zeks}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
             ...
             case 'tulis2':
                    gh = body.slice(8)
                    if (args.length < 1) return reply('Silahkan kirim perintah ${prefix}nulis2 nama|kelas|kode warna|teks\n\n*Kode Warna*:\n*Hitam*: 1\n*Merah*: 2\n*Biru*: 3')
                    if (!isRegister) return reply(mess.only.daftarB)
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
                    nama = gh.split("|")[0];
                    kelas = gh.split("|")[1];
                    tinta = gh.split("|")[2]
                    teks = gh.split("|")[3]
                    data = await fetchJson(`https://api.zeks.xyz/api/magernulis?nama=${nama}&kelas=${kelas}&text=${teks}&tinta=${tinta}`)
